<template>
  <div class="search">
    ...
    <button class="btn btn-primary search-btn" @click.prevent="getAvailableDocks">Search</button>
    <div id="ship" v-if="chooseShip & store.isLoggedIn">
      <label for="shipname"><strong>Ship's name:</strong> </label> <br />
      <select class="form-select" aria-label="Default select example" v-model="resData.ship" placeholder="Choose a ship">
        <option value="" hidden selected>Choose a ship</option>
        <option v-for="(ship, idx) in shipList" :key="idx" :value="ship.name">{{ship.name}}</option>
      </select>
    </div>
  </div>
